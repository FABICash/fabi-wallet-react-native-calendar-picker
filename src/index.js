...
export class CalendarPicker extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			...this.updateMonthYearWeekday(props.initialDate),
	...
	static defaultProps = {
		...
		onCalendarCancel: () => console.log('cancel the calendar'),
	};
	...
	componentDidUpdate(prevProps) {
		...
		let newMonthYear = {};
		if (
			!moment(prevProps.initialDate).isSame(this.props.initialDate, "day")
		) {
			newMonthYear = this.updateMonthYearWeekday(this.props.initialDate);
			doStateUpdate = true;
		}
	...
	updateMonthYearWeekday = (initialDate = this.props.initialDate, updateState) => {
		const newState = {
			currentMonth: parseInt(moment(initialDate).month()),
			currentYear: parseInt(moment(initialDate).year()),
			currentWeekday: this.state?.currentWeekday || parseInt(moment(initialDate).weekday())
		};
		if (updateState) {
	...
	handleOnPressDay = ({ year, month, day }) => {
		...
		if (
			...
			const dataAlreadySelected = !!prevSelectedStartDate && !!prevSelectedEndDate

			if (date.isSameOrAfter(prevSelectedStartDate, "day")) {
				const selectedStartDate = dataAlreadySelected ? null : prevSelectedStartDate;
				const selectedEndDate = dataAlreadySelected ? null : date;
				this.setState({
					selectedEndDate,
				...
				const selectedEndDate = dataAlreadySelected ? null : prevSelectedStartDate.clone();
				const selectedStartDate = dataAlreadySelected ? null : date;
				this.setState(
					{
						selectedStartDate,
						selectedEndDate,
			...
			const selectedStartDate = !!prevSelectedStartDate ? null : date;
	...
	createMonthProps = (state) => {
		return {
			...
			selectedDisabledDatesTextStyle: this.props
	...
	renderMonth = (props) => <DaysGridView {...props} />;
					...
					<View styles={styles.calendar}>
						...
						{scrollable ? (
							<Scroller
								...
								maxDate={maxDate}
								restrictMonthNavigation={restrictMonthNavigation}
								updateMonthYear={this.updateMonthYearWeekday}
